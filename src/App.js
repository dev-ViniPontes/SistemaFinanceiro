...
import { Menu, X, Home, Plus, Settings, Edit3 } from 'lucide-react';
...
function App() {
  const { data, updateData, adicionarReceita, adicionarItemClassificacao } = useLocalStorage();
  ...
  const renderTela = () => {
    switch (telaAtual) {
      ...
      case 'adicionar-receita':
        return <AdicionarReceita onAdicionarReceita={adicionarReceita} />;
      case 'gerenciar-gastos':
        return <GerenciarGastos data={data} onAdicionarItem={adicionarItemClassificacao} />;
      case 'configuracoes':
