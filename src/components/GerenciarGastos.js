...
const GerenciarGastos = ({ data, onAdicionarItem, onEditarItem, onExcluirItem }) => {
  ...
  const deveMostrarItem = (item) => {
    ...
    switch (item.tipo) {
      case 'parcelado':
        // Verificar se tem data de início e total de parcelas
        if (!item.dataInicio || !item.totalParcelas) return false;
        ...
        // O item deve aparecer apenas entre o mês de início e o total de parcelas
        return mesesDesdeInicio >= 0 && mesesDesdeInicio < item.totalParcelas;
      case 'gasto-fixo':
  ...
  const handleSubmit = (e) => {
    ...
    if (itemEditando) {
      ...
      onEditarItem(classificacaoSelecionada.id, { ...novoItem, id: itemEditando.id });
    } else {
    ...
    alert(`Item ${itemEditando ? 'editado' : 'adicionado'} com sucesso!`);
  };

  const abrirModal = (classificacao, item = null) => {
    ...
    if (item) {
      ...
      setFormData({
        ...
        valorParcela: item.valorParcela ? item.valorParcela.toString() : '',
        totalParcelas: item.totalParcelas ? item.totalParcelas.toString() : '',
        mesInicio: item.dataInicio ? item.dataInicio.slice(0, 7) : '',
        mesFechamento: item.mesFechamento || '',
        mesReferencia: item.mesReferencia || configuracoes.mesAtual
      });
    } else {
  ...
  const handleExcluirItem = (classificacaoId, itemId) => {
    if (window.confirm('Tem certeza que deseja excluir este item?')) {
      onExcluirItem(classificacaoId, itemId);
                ...
                {classif.itens.filter(item => item.ativo).map((item) => (
                  <div key={item.id} className="p-3 bg-gray-50 rounded-lg border">
                    ...
                    <div className="mt-3 flex justify-end space-x-2">
                      <button
                        onClick={() => abrirModal(classif, item)}
                      ...
                      <button
                        onClick={() => handleExcluirItem(classif.id, item.id)}
            ...
            <div className="p-6">
              <div className="flex items-center justify-between mb-6">
                <h3 className="text-lg font-semibold text-gray-900">
                  {itemEditando ? 'Editar Item' : 'Adicionar Item'} - {classificacaoSelecionada?.nome}
                </h3>
                <button
                  onClick={fecharModal}
